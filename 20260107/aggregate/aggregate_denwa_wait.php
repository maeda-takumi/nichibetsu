...
/* ===== Common helpers (aggregate_chosei と同等/独立) ===== */
function _dw_alias_map_from_actors_items(array $items): array {
  $map = [];
  foreach ($items as $a) {
...
function _dw_pick_actor_name(array $row): string {
...
function _dw_normalize_mdy_with_infer(string $md, array $row): string {
  ...
  if (preg_match('/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?$/', $md, $m)) {
    ...
    if ($yy === null) { [$iy, $im] = _dw_infer_year_month($row); $yy = $iy; }
...
/* ===== 抽出条件（電話対応待ち） =====
 * aggregate_chosei と同じ + 状態="電話対応待ち"
 */
function _dw_row_passes_filters(array $row, array $actorProfile): bool {
  ...
  $ok = false;
  foreach ($rowSystems as $rs) { if (in_array($rs, $allowSystems, true)) { $ok=true; break; } }
  if (!$ok) return false;

  // 状態 = 電話対応待ち
  $status = trim((string)($row['状態'] ?? ''));
  if ($status !== '電話対応待ち') return false;
...
function build_denwa_wait_by_day(string $DATA_DIR, string $CACHE_DIR): array {
  ...
  $aliasMap  = _dw_alias_map_from_actors_items($actorsList);
  $profiles  = _dw_actor_profiles($actorsList);

  _dbg_dw('raw rows count', count($rows));
  _dbg_dw('actors items count', count($actorsList));
  _dbg_dw('actor profiles (sample)', array_slice($profiles, 0, 10, true));

  foreach ($rows as $row) {
    $name = _dw_pick_actor_name($row);
    if ($name === '') continue;
